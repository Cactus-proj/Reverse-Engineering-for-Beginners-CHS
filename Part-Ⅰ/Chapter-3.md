...
编译器会生成`1.obj`文件，并连接到`1.exe`。在我们的例子当中，文件包含两个部分：`CONST`（放数据常量）和`_TEXT`（放代码）。
...
我们回到汇编代码，正如我们看到的，字符串是由0字节结束的，这代表着字符串。关于C/C++字符串：57.1.1

在代码部分，`_TEXT`，只有一个函数：main()。函数main()与大多数函数一样都有开始和结束的代码。
